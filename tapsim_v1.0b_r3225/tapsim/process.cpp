...
void Process::evaporation(const EvaporationOptions& options, const std::string& outputHeader, System_3d* system)
{
	...
	while (surfaceTable.nodes().size() > 0)
	{
...

		{
			...
			const Surface_3d::Nodeset::const_iterator node = Surface_3d::evap_findCandidate(options.evapMode,surfaceTable);
...

		{
			...
			if (options.fixedInitialPosition)
				...
				position = evap_initialPosition(resultsData.nodeIndex,system->geomTable);
